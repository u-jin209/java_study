...
public class Ex01Board {
    ...
    private static void printList(){
        if (ArrayUtil.isEmpty(array)){
            ...
            String message = "상세보기할 글의 번호나 뒤로 가실려면 0을 입력해 주세요";
            int userChoice = ScannerUtil.nextInt(SCANNER, message);
    ...
    private static void printOne(int id){
        ...
        String meaasge = "1. 수정  2. 삭제  3. 뒤로가기";
        int userChoice = ScannerUtil.nextInt(SCANNER, meaasge, 1, 3);
        if(userChoice ==1){
            update(id);
        }else if(userChoice ==2){
            delete(id);
    ...
    private  static void update(int id){
        ...
        Board b = ArrayUtil.get(array, ArrayUtil.indexOf(array,temp));
        String message = "새로운 제목을 입력해 주세요";
        b.setTitle(ScannerUtil.nextLine(SCANNER, message));

        message = "새로운 내용을 입력해 주세요";
        b.setContent(ScannerUtil.nextLine(SCANNER,message));
