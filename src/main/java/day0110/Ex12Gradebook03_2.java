...
public class Ex12Gradebook03_2 {
        ...
        while (true){
            ...
            if (action == 1){
                if(max < ARRAY_LENGTH){
                    ...
                    action = ScannerUtil.nextInt(SCANNER, message);
                    if (action ==1){
                        insertInfo(SCANNER,stu,position);
                        inputSwitch = true;
                        position ++;
    ...
    public static void insertInfo(Scanner scanner, Student[] stu,int num) {
        ...
        stu[num].korean = ScannerUtil.nextInt(scanner,message, SCORE_MIN,SCORE_MAX);

        message = "학생의 영어 점수를 입력해주세요";
        stu[num].english = ScannerUtil.nextInt(scanner,message, SCORE_MIN,SCORE_MAX);

        message = "학생의 수학 점수를 입력해주세요";
        stu[num].math =ScannerUtil.nextInt(scanner,message, SCORE_MIN,SCORE_MAX);
    }
